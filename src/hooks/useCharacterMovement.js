...
import {
  ...
  worldOriginPixels,
} from '../components/village/worldViewport'
import {
  decrementDiagnostic,
  incrementDiagnostic,
...
export function useCharacterMovement({ worldState, onMove, onMovementEnd, characterElementRef, worldElementRef, disabled = false } = {}) {
  ...
  const worldOrigin = useMemo(() => worldOriginPixels(bounds), [bounds])
  ...
  const paintPosition = useCallback((position, direction, isMoving) => {
    ...
    if (world) {
      const cameraScale = cameraScaleForViewport(window.innerWidth, window.innerHeight)
      world.style.setProperty('--character-x', position.x)
      world.style.setProperty('--character-y', position.y)
      world.style.setProperty('--camera-character-x', `${(position.x + worldOrigin.x) * cameraScale}px`)
      world.style.setProperty('--camera-character-y', `${(position.y + worldOrigin.y) * cameraScale}px`)
    }
  }, [characterElementRef, worldElementRef, worldOrigin.x, worldOrigin.y])

  const stopAnimationFrame = useCallback(() => {
    if (!animationFrameRef.current) return
    window.cancelAnimationFrame(animationFrameRef.current)
    animationFrameRef.current = null
    decrementDiagnostic('activeRafLoops')
    incrementDiagnostic('rafStops')
  }, [])
  ...
  }, [paintPosition, playerPosition, stopAnimationFrame])
  ...
  const moveContextRef = useRef(null)
  ...
  const requestMove = useCallback(async (direction) => {
    const {
      ...
      paintPosition: currentPaintPosition,
      syncAfterMovement: currentSyncAfterMovement,
      terrainTiles: currentTerrainTiles,
    } = moveContextRef.current
    if (disabledRef.current || !currentOnMove || moveLockRef.current || !serverPositionRef.current) return null
    ...
    if (currentBounds && (targetX < currentBounds.minX || targetX > currentBounds.maxX || targetY < currentBounds.minY || targetY > currentBounds.maxY)) return null
    const tile = currentTerrainTiles.find((value) => value.x === targetX && value.y === targetY)
    if (!tile?.walkable) return null
    moveLockRef.current = true
    try {
      const result = await currentOnMove(targetX, targetY)
      if (!Number.isInteger(result?.currentX) || !Number.isInteger(result?.currentY) || !result.accepted) {
        ...
        if (approved) {
          ...
          currentPaintPosition(restored, direction, false)
      ...
      const duration = reducedMotion ? 0 : MOVE_ANIMATION_MS
      ...
      const animate = (now) => {
        ...
        renderPositionRef.current = next
        currentPaintPosition(next, direction, progress < 1)
        if (progress < 1) animationFrameRef.current = window.requestAnimationFrame(animate)
          ...
          currentSyncAfterMovement()
        }
      }
      incrementDiagnostic('activeRafLoops', 'maxActiveRafLoops')
      incrementDiagnostic('rafStarts')
  ...
  const scheduleMovement = useCallback(() => {
    ...
    incrementDiagnostic('activeMovementSchedulers', 'maxActiveMovementSchedulers')
    incrementDiagnostic('movementSchedulerStarts')
    const run = async () => {
      const direction = activeDirection()
      if (!direction) {
        schedulerRef.current = null
        schedulerActiveRef.current = false
        decrementDiagnostic('activeMovementSchedulers')
        return
      }
  ...
  const cancelMovement = useCallback(() => {
    ...
    if (schedulerActiveRef.current) decrementDiagnostic('activeMovementSchedulers')
    schedulerActiveRef.current = false
    movementEndPendingRef.current = false
    stopAnimationFrame()
  }, [stopAnimationFrame])
  ...
  useEffect(() => {
    incrementDiagnostic('keyboardHandlers', 'maxKeyboardHandlers')
    ...
    const onKeyDown = (event) => { const direction = keys[event.key]; if (disabledRef.current || !direction || event.repeat || event.target?.matches?.('input,textarea,[contenteditable=true]')) return; event.preventDefault(); keyboardDirectionsRef.current = [...keyboardDirectionsRef.current.filter((value) => value !== direction), direction]; scheduleMovement() }
    const onKeyUp = (event) => {
    ...
    return () => {
      ...
      decrementDiagnostic('keyboardHandlers')
      cancelMovement()
    }
